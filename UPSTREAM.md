...
| `pir/setup.rs` | **CORRECTED 2026-09-12:** the first inventory counted five removed fields; the fork diff contains six because `inspiring_precomp` was removed with `crs_a_vectors`, `k_g`, `k_h`, `packing_k_h`, and `packing_k_g`. The InspiRING-active bit derives from `inspiring_num_columns > 0`, and live packing derives keys from `inspiring_w_seed`. Shrinks the client-shipped CRS ~34.9 -> ~1.1 MiB. Breaking change. The CRS is wrapped with a `RAVEN_CRS_v03` magic prefix (`to_versioned_bytes`) and a `DECODE_LIMIT_BYTES` length cap; older layouts fail the magic check on load and require re-bootstrap. |
| `math/poly.rs`, `pir/respond.rs` | Binary coefficient vectors use a lossless modulus-width codec with bounded allocation; human-readable serde retains coefficient arrays. |
